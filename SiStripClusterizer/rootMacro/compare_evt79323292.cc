...
{
	TDirectoryFile* _;
	TFile* raw_c   = TFile::Open("../output/raw_RAW2DIGI_L1Reco_RECO_clusterNtuple.root", "read");
	_  = (TDirectoryFile*) raw_c->Get("SiStripClustersDump");
	TTree* t_r_c   = (TTree*) _->Get("clusters");
	
	TFile* rawp_c  = TFile::Open("../output/rawprime_RAW2DIGI_L1Reco_RECO_clusterNtuple.root", "read");
	_  = (TDirectoryFile*) rawp_c->Get("SiStripClustersDump");
	TTree* t_rp_c  = (TTree*) _->Get("clusters");
	
	TFile* rawp_ac = TFile::Open("../output/rawprime_RAW2DIGI_L1Reco_RECO_approxClusterNtuple.root", "read");
